#!/usr/bin/env node
var bitbulljs = require("bitbulljs");
...
var Ledgerbitbull = require('./src/Ledgerbitbull.js');
...
function postTransaction(container, transaction, cb){
  ...
  let senderAddress = bitbulljs.crypto.getAddress(transaction.senderPublicKey);
      ...
      if (body.account.secondSignature) {
        container.prompt({
          ...
          if (result.passphrase) {
            var secondKeys = bitbulljs.crypto.getKeys(result.passphrase);
            bitbulljs.crypto.secondSign(transaction, secondKeys);
            transaction.id = bitbulljs.crypto.getId(transaction);
            performPost();
          } else {
...
function getbitbullTicker(currency){
  request({url: "https://api.coinmbitbulletcap.com/v1/ticker/bitbull/?convert="+currency}, function(err, response, body){
    bitbullticker[currency]=JSON.parse(body)[0];
  });
}
...
async function populateLedgerAccounts() {
  ...
  while (!empty) {
    ...
    try {
      ...
      if (result.publicKey) {
        result.address = bitbulljs.crypto.getAddress(result.publicKey);
...
async function ledgerSignTransaction(seriesCb, transaction, account, callback) {
  ...
  var transactionHex = bitbulljs.crypto.getBytes(transaction, true, true).toString("hex");
  var result = null;
...
ledgerWorker.on('message', function (message) {
  if (message.connected && network && (!ledgerComm || !ledgerAccounts.length)) {
    ledger.comm_node.create_async().then((comm) => {
      ledgerComm = comm;
      ledgerBridge = new Ledgerbitbull(ledgerComm);
  ...
  .action(function(args, callback) {
    ...
    connect2network(network,function(){
      getFromNode('http://'+server+'/peer/status', function(err, response, body){
        ...
        self.delimiter('bitbull '+args.network+'>');
        bitbulljs.crypto.setNetworkVersion(network.config.version);
        callback();
      });
    });
  ...
  .action(function(args, callback) {
    ...
    if(!server){
      ...
      if(err){
        ...
        var spinner = ora({text:"0%",spinner:"shbitbull"}).start();
        var heights={};
        var delays={};
        var count=0;
    ...
    async.waterfall([
      ...
      function(account, seriesCb) {
        ...
        if (account.passphrase) {
          ...
          var keys = bitbulljs.crypto.getKeys(passphrase);
          publicKey = keys.publicKey;
          address = bitbulljs.crypto.getAddress(publicKey);
        ...
        getFromNode('http://'+server+'/api/accounts/delegates/?address='+address, function(err, response, body) {
          ...
          getFromNode('http://'+server+'/api/delegates/get/?username='+delegateName, function(err, response, body){
            ...
            self.prompt({
              ...
              if (result.continue) {
                if (currentVote) {
                  var unvoteTransaction = bitbulljs.vote.createVote(passphrase, ['-'+currentVote.publicKey]);
                      ...
                      var checkTransactionTimerId = setInterval(function() {
                        getFromNode('http://' + server + '/api/transactions/get?id=' + transactionId, function(err, response, body) {
                          ...
                          if (!body.success && body.error !== 'Transaction not found') {
                            ...
                            clearInterval(checkTransactionTimerId);
                            var transaction = bitbulljs.vote.createVote(passphrase, ['+'+newDelegate.publicKey]);
      ...
      function(account, seriesCb){
        ...
        getFromNode('http://'+server+'/api/accounts/delegates/?address='+address, function(err, response, body) {
          ...
          self.prompt({
            ...
            if (result.continue) {
              var transaction = bitbulljs.vote.createVote(passphrase, delegates);
              ledgerSignTransaction(seriesCb, transaction, account, function(transaction) {
                if (!transaction) {
                  return seriesCb('Failed to sign transaction with ledger');
                }
...
vorpal
  .command('account send <amount> <address>', 'Send <amount> bitbull to <address>. <amount> format examples: 10, USD10.4, EUR100')
  .action(function(args, callback) {
		var self = this;
    if(!server){
      self.log("please connect to node or network before");
    ...
    async.waterfall([
      ...
      function(account, seriesCb){
        ...
        var bitbullamount = args.amount;
        var bitbullAmountString = args.amount;

        if(currency){
          if(!bitbullticker[currency]){
            return seriesCb("Can't get price from mbitbullet. Aborted.");
          }
          bitbullamount = parseInt(args.amount * 100000000 / Number(bitbullticker[currency]["price_"+currency.toLowerCase()]))
          bitbullAmountString = bitbullamount/100000000;
        }
        ...
        self.prompt({
          ...
          name: 'continue',
          default: false,
          message: 'Sending '+bitbullAmountString+network.config.token+' '+(currency?'('+currency+args.amount+') ':'')+'to '+args.address+' now',
        }, function(result){
          if (result.continue) {
            var transaction = bitbulljs.transaction.createTransaction(args.address, bitbullamount, null, passphrase);
            ledgerSignTransaction(seriesCb, transaction, account, function(transaction) {
              if (!transaction) {
                return seriesCb('Failed to sign transaction with ledger');
              }
  ...
  .action(function(args, callback) {
    ...
    async.waterfall([
      ...
      function(account, seriesCb) {
        ...
        var transaction = bitbulljs.delegate.createDelegate(passphrase, args.username);
        ledgerSignTransaction(seriesCb, transaction, account, function(transaction) {
          if (!transaction) {
            return seriesCb('Failed to sign transaction with ledger');
          }
...
vorpal
  ...
  .action(function(args, callback) {
    ...
    var passphrase = require("bip39").generateMnemonic();
		...
		self.log("WIF     - private:",bitbulljs.crypto.getKeys(passphrase).toWIF());
		self.log("Address - public :",bitbulljs.crypto.getAddress(bitbulljs.crypto.getKeys(passphrase).publicKey));
		callback();
  });
...
vorpal
  ...
  .action(function(args, callback) {
    ...
    var spinner = ora({text:"passphrases tested: 0",spinner:"shbitbull"}).start();
  ...
  .action(function(args, callback) {
    ...
    return this.prompt({
      ...
      if (result.passphrase) {
        ...
        self.log("public key: ",bitbulljs.crypto.getKeys(result.passphrase).publicKey);
        self.log("address   : ",bitbulljs.crypto.getAddress(bitbulljs.crypto.getKeys(result.passphrase).publicKey));
        self.log("signature : ",bitbulljs.crypto.getKeys(result.passphrase).sign(hash).toDER().toString("hex"));
      ...
      if (result.signature) {
        try{
          ...
          var signature = new Buffer(result.signature, "hex");
        	...
        	var ecpair = bitbulljs.ECPair.fromPublicKeyBuffer(publickey);
        	var ecsignature = bitbulljs.ECSignature.fromDER(signature);
...
var shbitbullspinner;
vorpal
  .command("shbitbull", "No you don't want to use this command")
  .action(function(args, callback) {
		var self = this;
    self.log(colors.red(figlet.textSync("shbitbull")));
    shbitbullspinner = ora({text:"Watch out, the shbitbull attack!",spinner:"shbitbull"}).start();
    callback();
  });

vorpal
  .command("spbitbullaaaaa!")
  .hidden()
  .action(function(args, callback) {
    var time = 0;
    var self=this;
    shbitbullspinner && shbitbullspinner.stop();
    ["tux","meow","bunny","cower","dragon-and-cow"].forEach(function(spbitbull){
      setTimeout(function(){
        self.log(cowsay.say({text:"SPAAAAbitbullKKAAAAAAA!", f:spbitbull}));
  		}, time++*1000);
    });
...
vorpal.history('bitbull-client');

vorpal.log(colors.cyan(figlet.textSync("bitbull Client","Slant")));
